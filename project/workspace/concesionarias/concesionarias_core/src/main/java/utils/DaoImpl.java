...
import annotations.NoEntityException;
...
public abstract class DaoImpl<T> {
    ...
    private static String getProcedureWithParameters(final String procedure_call_query, final Stream<String> params) {
        final String procedureCallQuery = procedure_call_query + "(TOKEN)";
        final String reduce = params.reduce("", (acc, param) ->
                acc.equals("") ? acc + "?" : acc + ",?"
        );
        return procedureCallQuery.replaceAll("TOKEN", reduce);
    }
    ...
    public void executeProcedureSetter(final String procedureName, final T bean) throws SQLException {
        this.connect();
        final Stream<String> columnNames =
                Stream.of(bean.getClass().getFields()).map(Field::getName);

        final String procedureWithParameters = getProcedureWithParameters(procedureName, columnNames);
        this.setProcedure(procedureWithParameters);
        final ArrayList<String> collect = columnNames.collect(Collectors.toCollection(ArrayList::new));
        final String[] columnNamesVector = (String[]) collect.toArray();
        setParameters(bean, columnNamesVector);
        this.executeUpdate();
        this.disconnect();
    }
